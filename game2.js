(() => {
  ...
  const games = [
    ...
    { id:"database", label:"Database", icon:"fas fa-database", color:"#ffd166" },
    { id:"api", label:"REST API", icon:"fas fa-code-branch", color:"#8fa7ff" },
  ...
  let first = null, lock = false, moves = 0, matched = 0, seconds = 0, timer = null, started = false;
  ...
  const finish = () => {
    ...
    if (!savedBest || moves < Number(localStorage.getItem(bestKey) || Infinity)) {
  ...
  const flip = card => {
    ...
    if (!started) {
      started = true;
      timer = setInterval(() => { seconds += 1; timeEl.textContent = `${seconds}s`; }, 1000);
    }
    card.classList.add("flipped");
    if (!first) { first = card; message.textContent = "Now find its matching technology."; return; }
    ...
    if (first.dataset.id === card.dataset.id) {
      ...
      first = null; matched += 1; message.textContent = "Match found!";
      if (matched === games.length) finish();
    } else {
      lock = true; message.textContent = "Not a match - try again.";
      const previous = first; first = null;
      setTimeout(() => {
        previous.classList.remove("flipped"); card.classList.remove("flipped"); lock = false;
      }, 720);
    }
  };
  const build = () => {
    clearInterval(timer); timer = null; board.innerHTML = ""; first = null; lock = false;
    moves = 0; matched = 0; seconds = 0; started = false; movesEl.textContent = "0"; timeEl.textContent = "0s";
    message.textContent = "Choose any card to begin.";
    shuffle([...games, ...games].map((game, index) => ({ ...game, key:`${game.id}-${index}` }))).forEach(game => {
      const card = document.createElement("button");
      card.type = "button"; card.className = "memory-card"; card.dataset.id = game.id;
