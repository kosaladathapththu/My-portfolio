const styles = {
  futuristic:
    "futuristic cinematic technology, electric blue and warm amber lighting, premium glass interfaces, subtle circuit patterns",
  minimal:
    "minimal premium technology, deep navy background, clean geometric forms, elegant blue and gold accents",
  cyber:
    "cyberpunk software engineering studio, neon cyan and magenta light, holographic dashboards, cinematic depth",
};
...
export default async (request) => {
  ...
  try {
    ...
    if (prediction.status !== "succeeded" || !prediction.output) {
      return Response.json(
        { error: "Image generation did not finish in time" },
        { status: 504 },
      );
    }
    const image = Array.isArray(prediction.output)
      ? prediction.output[0]
      : prediction.output;
    ...
    return Response.json(
      { error: "Image generator temporarily unavailable" },
      { status: 500 },
    );
  }
};
