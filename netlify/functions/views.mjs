  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      if (!VISITOR_ID.test(visitorId)) {
        return new Response(JSON.stringify({ error: "Invalid visitor identifier" }), { status: 400, headers });
      }
    ...
    return new Response(JSON.stringify({ views: blobs.length }), { status: 200, headers });
  } catch (error) {
    console.error("Profile view counter error:", error);
    return new Response(JSON.stringify({ error: "View counter temporarily unavailable" }), { status: 500, headers });
  }
};
